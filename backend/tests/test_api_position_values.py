...
class TestPositionValuesAPI:
    ...
    def test_get_position_value_by_isin(self, client):
        """Test getting a specific position value by ISIN."""
        # Create position value
        client.post("/api/v1/position-values", json={"isin": "IE00B4L5Y983", "current_value": "5000.50"})

        # Get by ISIN
        response = client.get("/api/v1/position-values/IE00B4L5Y983")
    ...
    def test_get_position_value_not_found(self, client):
        """Test getting non-existent position value returns 404."""
        response = client.get("/api/v1/position-values/NONEXISTENT1")

        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()

    def test_delete_position_value(self, client):
        """Test deleting a position value."""
        # Create position value
        client.post("/api/v1/position-values", json={"isin": "IE00B4L5Y983", "current_value": "5000.50"})

        # Delete
        response = client.delete("/api/v1/position-values/IE00B4L5Y983")

        assert response.status_code == 204

        # Verify deleted
        get_response = client.get("/api/v1/position-values/IE00B4L5Y983")
        assert get_response.status_code == 404

    def test_delete_position_value_not_found(self, client):
        """Test deleting non-existent position value returns 404."""
        response = client.delete("/api/v1/position-values/NONEXISTENT1")
